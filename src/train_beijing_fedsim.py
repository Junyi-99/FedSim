...
[X1, X2], y = load_both(
    house_path=house_dataset, airbnb_path=airbnb_dataset, active_party="house"
)
...
prof = torch.profiler.profile(
    ...
    # on_trace_ready=torch.profiler.tensorboard_trace_handler("./tf_profile/beijing"),
    record_shapes=False,
...
model.train_splitnn(
    ...
    X2,
    y,
    data_cache_path=os.path.join(cache_path, "beijing_sim.pkl"),
    scale=True,
    prof=prof,
    use_optimized_dataset=args.optimized_dataset,
)
...
#     )
# )
prof.export_chrome_trace(os.path.join(cache_path, "fedsim_trace_50.json"))
